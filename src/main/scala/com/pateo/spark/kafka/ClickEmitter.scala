...
object ClickEmitter extends App {
  if (args.length < 3) {
    System.err.println("Usage: KafkaWordCountProducer <metadataBrokerList> <topic> <producespeed>")
    System.exit(1)
  }

  val Array(brokers, topic, producespeed ) = args
  ...
  while(true) {
    ...
    Thread.sleep(producespeed.toInt)
  }
